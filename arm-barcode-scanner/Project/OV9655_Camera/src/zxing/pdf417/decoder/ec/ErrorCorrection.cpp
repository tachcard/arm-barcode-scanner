...
void ErrorCorrection::decode(ArrayRef<int> received, int numECCodewords,
	...
	if (error) {
		...
		vector<Ref<ModulusPoly> > sigmaOmega(
				runEuclideanAlgorithm(field_.buildMonomial(numECCodewords, 1),
		...
		ArrayRef<int> errorMagnitudes = findErrorMagnitudes(omega, sigma,
				errorLocations);
		...
		for (int i = 0; i < errorLocations->size(); i++) {
			...
			received[position] = field_.subtract(received[position],
					errorMagnitudes[i]);
...
vector<Ref<ModulusPoly> > ErrorCorrection::runEuclideanAlgorithm(
		Ref<ModulusPoly> a, Ref<ModulusPoly> b, int R) {
	...
	while (r->getDegree() >= R / 2) {
		...
		while (r->getDegree() >= rLast->getDegree() && !r->isZero()) {
			...
			int scale = field_.multiply(r->getCoefficient(r->getDegree()),
					dltInverse);
...
ArrayRef<int> ErrorCorrection::findErrorLocations(
...
ArrayRef<int> ErrorCorrection::findErrorMagnitudes(
		Ref<ModulusPoly> errorEvaluator, Ref<ModulusPoly> errorLocator,
		ArrayRef<int> errorLocations) {
	int i;
	int errorLocatorDegree = errorLocator->getDegree();
	ArrayRef<int> formalDerivativeCoefficients(
			new Array<int>(errorLocatorDegree));
	for (i = 1; i <= errorLocatorDegree; i++) {
		formalDerivativeCoefficients[errorLocatorDegree - i] = field_.multiply(
				i, errorLocator->getCoefficient(i));
	}
	Ref<ModulusPoly> formalDerivative(
			new ModulusPoly(field_, formalDerivativeCoefficients));
	...
	for (i = 0; i < s; i++) {
		...
		int numerator = field_.subtract(0,
				errorEvaluator->evaluateAt(xiInverse));
		int denominator = field_.inverse(
