...
#include <zxing/pdf417/decoder/Pdf417Decoder.h>
#include <zxing/pdf417/decoder/Pdf417BitMatrixParser.h>
#include <zxing/pdf417/decoder/Pdf417DecodedBitStreamParser.h>
...
void Decoder::verifyCodewordCount(ArrayRef<int> codewords, int numECCodewords) {
	...
	if (cwsize < 4) {
		...
		throw FormatException(
				"PDF:Decoder:verifyCodewordCount: codeword array too small!");
	}
	...
	if (numberOfCodewords > cwsize) {
		throw FormatException(
				"PDF:Decoder:verifyCodewordCount: bad codeword number descriptor!");
	}
 ...
 * @throws FormatException
 */
void Decoder::correctErrors(ArrayRef<int> codewords, ArrayRef<int> erasures,
		int numECCodewords) {
	if (erasures->size() > numECCodewords / 2 + MAX_ERRORS || numECCodewords < 0
	...
	for (int i = 0; i < codewords->size(); i++) {
		if (codewords[i] < 0) {
			throw FormatException(
					"PDF:Decoder:correctErrors: Error correction did not succeed!");
		}
	}
}
