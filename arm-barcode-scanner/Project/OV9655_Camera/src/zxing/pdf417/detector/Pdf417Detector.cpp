...
#include <limits>
#include <zxing/pdf417/detector/Pdf417Detector.h>
...
const int Detector::MAX_AVG_VARIANCE = (int) (PATTERN_MATCH_RESULT_SCALE_FACTOR
		* 0.42f);
const int Detector::MAX_INDIVIDUAL_VARIANCE =
		(int) (PATTERN_MATCH_RESULT_SCALE_FACTOR * 0.8f);
...
const int Detector::START_PATTERN_REVERSE_LENGTH = sizeof(START_PATTERN_REVERSE)
		/ sizeof(int);
...
const int Detector::STOP_PATTERN_REVERSE_LENGTH = sizeof(STOP_PATTERN_REVERSE)
		/ sizeof(int);
...
Ref<DetectorResult> Detector::detect(DecodeHints const& hints) {
	...
	int dimension = computeDimension(vertices[12], vertices[14], vertices[13],
			vertices[15], moduleWidth);
	if (dimension < 1) {
		throw NotFoundException("Bad dimension.");
	}

	int yDimension = max(
			computeYDimension(vertices[12], vertices[14], vertices[13],
					vertices[15], moduleWidth), dimension);
...
ArrayRef<Ref<ResultPoint> > Detector::findVertices(Ref<BitMatrix> matrix,
	...
	for (int i = 0; i < height; i += rowStep) {
		ArrayRef<int> loc = findGuardPattern(matrix, 0, i, width, false,
				START_PATTERN, START_PATTERN_LENGTH, counters);
		if (loc) {
	...
	if (found) { // Found the Bottom Left vertex
		...
		for (int i = 0; i < height; i += rowStep) {
			ArrayRef<int> loc = findGuardPattern(matrix, 0, i, width, false,
					STOP_PATTERN, STOP_PATTERN_LENGTH, counters);
			if (loc) {
...
ArrayRef<Ref<ResultPoint> > Detector::findVertices180(Ref<BitMatrix> matrix,
		int rowStep) {
	const int height = matrix->getHeight();
	const int width = matrix->getWidth();
	...
	// Top Left
	for (int i = height - 1; i > 0; i -= rowStep) {
		ArrayRef<int> loc = findGuardPattern(matrix, halfWidth, i, halfWidth,
				true, START_PATTERN_REVERSE, START_PATTERN_REVERSE_LENGTH,
				counters);
		if (loc) {
	...
	if (found) { // Found the Top Left vertex
		found = false;
		for (int i = 0; i < height; i += rowStep) {
			ArrayRef<int> loc = findGuardPattern(matrix, halfWidth, i,
					halfWidth, true, START_PATTERN_REVERSE,
	...
	if (found) { // Found the Bottom Left vertex
		...
		for (int i = height - 1; i > 0; i -= rowStep) {
			ArrayRef<int> loc = findGuardPattern(matrix, 0, i, halfWidth, false,
					STOP_PATTERN_REVERSE, STOP_PATTERN_REVERSE_LENGTH,
					counters);
			if (loc) {
 ...
 */
ArrayRef<int> Detector::findGuardPattern(Ref<BitMatrix> matrix, int column,
		int row, int width, bool whiteFirst, const int pattern[],
		int patternSize, ArrayRef<int>& counters) {
	...
	for (int x = 0; x < numCounters; x++) {
		...
		int variance =
				counter > scaledPattern ?
						counter - scaledPattern : scaledPattern - counter;
...
void Detector::correctVertices(Ref<BitMatrix> matrix,
		ArrayRef<Ref<ResultPoint> >& vertices, bool upsideDown) {
	...
	if (isLowLeft || isLowRight) {
		...
		findWideBarTopBottom(matrix, vertices, 0, 0, 8, 17,
				upsideDown ? 1 : -1);
		findWideBarTopBottom(matrix, vertices, 1, 0, 8, 17,
				upsideDown ? -1 : 1);
		findWideBarTopBottom(matrix, vertices, 2, 11, 7, 18,
				upsideDown ? 1 : -1);
		findWideBarTopBottom(matrix, vertices, 3, 11, 7, 18,
				upsideDown ? -1 : 1);
...
void Detector::findWideBarTopBottom(Ref<BitMatrix> matrix,
		ArrayRef<Ref<ResultPoint> > &vertices, int offsetVertice,
		int startWideBar, int lenWideBar, int lenPattern, int rowStep) {
	...
	float barStart = verticeStart->getX()
			+ barDiff * (float) startWideBar / (float) lenPattern;
	float barEnd = verticeStart->getX()
			+ barDiff * (float) endWideBar / (float) lenPattern;
	...
	while (!isEnd) {
		if (matrix->get(x, y)) {
			...
			isEnd = !matrix->get(x + nextBarX, y)
					&& !matrix->get(x + nextBarX + 1, y);
			y += rowStep;
			...
			if (x > 0 && matrix->get(x - 1, y)) {
				x--;
			} else if (x < (int) matrix->getWidth() - 1
					&& matrix->get(x + 1, y)) {
				x++;
			} else {
...
void Detector::findCrossingPoint(ArrayRef<Ref<ResultPoint> >& vertices,
		int idxResult, int idxLineA1, int idxLineA2, int idxLineB1,
		int idxLineB2, Ref<BitMatrix>& matrix) {
	...
	if (result.x == numeric_limits<float>::infinity()
		...
		throw NotFoundException(
				"PDF:Detector: cannot find the crossing of parallel lines!");
	}

	int x = Math::round(result.x);
	int y = Math::round(result.y);
	if (x < 0 || x >= (int) matrix->getWidth() || y < 0
			|| y >= (int) matrix->getHeight()) {
...
int Detector::computeDimension(Ref<ResultPoint> const& topLeft,
		Ref<ResultPoint> const& topRight, Ref<ResultPoint> const& bottomLeft,
		Ref<ResultPoint> const& bottomRight, float moduleWidth) {
	int topRowDimension = MathUtils::round(
			ResultPoint::distance(topLeft, topRight) / moduleWidth);
	int bottomRowDimension = MathUtils::round(
			ResultPoint::distance(bottomLeft, bottomRight) / moduleWidth);
...
int Detector::computeYDimension(Ref<ResultPoint> const& topLeft,
		Ref<ResultPoint> const& topRight, Ref<ResultPoint> const& bottomLeft,
		Ref<ResultPoint> const& bottomRight, float moduleWidth) {
	int leftColumnDimension = MathUtils::round(
			ResultPoint::distance(topLeft, bottomLeft) / moduleWidth);
	int rightColumnDimension = MathUtils::round(
			ResultPoint::distance(topRight, bottomRight) / moduleWidth);
 ...
 */
Ref<BitMatrix> Detector::sampleLines(
		ArrayRef<Ref<ResultPoint> > const& vertices, int dimensionY,
		int dimension) {
	const int sampleDimensionX = dimension * 8;
	const int sampleDimensionY = dimensionY * 4;
	Ref<PerspectiveTransform> transform(
			PerspectiveTransform::quadrilateralToQuadrilateral(0.0f, 0.0f,
					(float) sampleDimensionX, 0.0f, 0.0f,
					(float) sampleDimensionY, (float) sampleDimensionX,
					(float) sampleDimensionY, vertices[12]->getX(),
	...
	Ref<BitMatrix> linesMatrix = GridSampler::getInstance().sampleGrid(
			image_->getBlackMatrix(), sampleDimensionX, sampleDimensionY,
			transform);
