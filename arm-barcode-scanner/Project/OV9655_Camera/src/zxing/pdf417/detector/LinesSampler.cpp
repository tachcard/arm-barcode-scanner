...
#include <zxing/pdf417/decoder/Pdf417BitMatrixParser.h>
...
vector<float> LinesSampler::init_ratios_table() {
	...
	vector<float> RATIOS_TABLE(
			BitMatrixParser::SYMBOL_TABLE_LENGTH
					* LinesSampler::BARS_IN_SYMBOL);
	int x = 0;
...
Ref<BitMatrix> LinesSampler::sample() {
	...
	linesMatrixToCodewords(clusterNumbers, symbolsPerLine, symbolWidths,
			linesMatrix_, codewords);

	// XXX
	vector < vector<map<int, int> > > votes = distributeVotes(symbolsPerLine,
			codewords, clusterNumbers);
	...
	for (int i = 0; i < (int) votes.size(); i++) {
		...
		for (int j = 0; j < (int) votes[i].size(); j++) {
			if (!votes[i][j].empty()) {
				detectedCodeWords[i][j] =
						getValueWithMaxVotes(votes[i][j]).getVote();
			}
		}
	}

	// XXX
	vector<int> insertLinesAt = findMissingLines(symbolsPerLine,
			detectedCodeWords);

	// XXX
	int rowCount = decodeRowCount(symbolsPerLine, detectedCodeWords,
			insertLinesAt);
 ...
 * @param matrix
 */
void LinesSampler::codewordsToBitMatrix(vector<vector<int> > &codewords,
	...
	for (int i = 0; i < (int) codewords.size(); i++) {
		for (int j = 0; j < (int) codewords[i].size(); j++) {
			...
			for (int k = 0; k < MODULES_IN_SYMBOL; k++) {
				if ((codewords[i][j] & (1 << (MODULES_IN_SYMBOL - k - 1)))
						> 0) {
...
void LinesSampler::computeSymbolWidths(vector<float> &symbolWidths,
		const int symbolsPerLine, Ref<BitMatrix> linesMatrix) {
	int symbolStart = 0;
	bool lastWasSymbolStart = true;
	const float symbolWidth =
			symbolsPerLine > 0 ?
					(float) linesMatrix->getWidth() / (float) symbolsPerLine :
...
void LinesSampler::linesMatrixToCodewords(vector<vector<int> >& clusterNumbers,
		const int symbolsPerLine, const vector<float>& symbolWidths,
		Ref<BitMatrix> linesMatrix, vector<vector<int> >& codewords) {
	for (int y = 0; y < linesMatrix->getHeight(); y++) {
		// Not sure if this is the right way to handle this but avoids an error:
		if (symbolsPerLine > (int) symbolWidths.size()) {
			throw NotFoundException(
					"Inconsistent number of symbols in this line.");
		}
		...
		for (int i = 0; i < symbolsPerLine; i++) {
			...
			const int cwEnd =
					(i == symbolsPerLine - 1) ? barCount : cwStarts[i + 1];
			...
			if (cwLength == 7) {
				for (int j = 0; j < cwLength; ++j) {
					cwRatios[i][j] = (float) barWidths[cwStart + j]
							/ symbolWidths[i];
				}
			...
			for (int j = 0; j < POSSIBLE_SYMBOLS; j++) {
				...
				for (int k = 0; k < BARS_IN_SYMBOL; k++) {
					float diff = RATIOS_TABLE[j * BARS_IN_SYMBOL + k]
							- cwRatios[i][k];
					error += diff * diff;
...
vector<vector<map<int, int> > > LinesSampler::distributeVotes(
		const int symbolsPerLine, const vector<vector<int> >& codewords,
			...
			if ((lineClusterNumber == 0 && lastLineClusterNumber == -1)
				...
				for (int i = 0; i < (int) codewords[y].size(); i++) {
					if (clusterNumbers[y][i] != -1) {
						...
						} else if (clusterNumbers[y][i]
								== ((lineClusterNumber + 3) % 9)) {
...
vector<int> LinesSampler::findMissingLines(const int symbolsPerLine,
	...
	if (detectedCodeWords.size() > 1) {
		for (int i = 0; i < (int) detectedCodeWords.size() - 1; i++) {
			...
			for (int j = 0;
					j < (int) detectedCodeWords[i].size()
							&& clusterNumberRow == -1; j++) {
				int clusterNumber = calculateClusterNumber(
						detectedCodeWords[i][j]);
				if (clusterNumber != -1) {
			...
			for (int j = 0;
					j < (int) detectedCodeWords[i + 1].size()
							&& clusterNumberNextRow == -1; j++) {
				int clusterNumber = calculateClusterNumber(
						detectedCodeWords[i + 1][j]);
				if (clusterNumber != -1) {
					clusterNumberNextRow = clusterNumber;
				}
			}
			if ((clusterNumberRow + 3) % 9 != clusterNumberNextRow
					&& clusterNumberRow != -1 && clusterNumberNextRow != -1) {
	...
	for (int i = 0; i < (int) insertLinesAt.size(); i++) {
		detectedCodeWords.insert(
				detectedCodeWords.begin() + insertLinesAt[i] + i,
...
int LinesSampler::decodeRowCount(const int symbolsPerLine,
		vector<vector<int> > &detectedCodeWords, vector<int> &insertLinesAt) {
	...
	for (int i = 0; i + 2 < (int) detectedCodeWords.size(); i += 3) {
		...
		if (detectedCodeWords[i][0] != 0) {
			firstCodewordDecodedLeft = BitMatrixParser::getCodeword(
					detectedCodeWords[i][0]);
		}
		if (detectedCodeWords[i + 1][0] != 0) {
			secondCodewordDecodedLeft = BitMatrixParser::getCodeword(
					detectedCodeWords[i + 1][0]);
		}
		if (detectedCodeWords[i + 2][0] != 0) {
			thirdCodewordDecodedLeft = BitMatrixParser::getCodeword(
					detectedCodeWords[i + 2][0]);
		}
		...
		if (detectedCodeWords[i + 1][detectedCodeWords[i + 1].size() - 1]
				!= 0) {
			secondCodewordDecodedRight =
					BitMatrixParser::getCodeword(
							detectedCodeWords[i + 1][detectedCodeWords[i + 1].size()
									- 1]);
		}
		if (detectedCodeWords[i + 2][detectedCodeWords[i + 2].size() - 1]
				!= 0) {
			thirdCodewordDecodedRight =
					BitMatrixParser::getCodeword(
							detectedCodeWords[i + 2][detectedCodeWords[i + 2].size()
									- 1]);
		}

		if (firstCodewordDecodedLeft != -1 && secondCodewordDecodedLeft != -1) {
			int leftRowCount = ((firstCodewordDecodedLeft % 30) * 3)
					+ ((secondCodewordDecodedLeft % 30) % 3);
