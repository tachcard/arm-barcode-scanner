...
vector<Ref<Result> > GenericMultipleBarcodeReader::decodeMultiple(
...
void GenericMultipleBarcodeReader::doDecodeMultiple(Ref<BinaryBitmap> image,
		DecodeHints hints, vector<Ref<Result> >& results, int xOffset,
	...
	for (unsigned int i = 0; i < results.size(); i++) {
		...
		if (existingResult->getText()->getText()
...
Ref<Result> GenericMultipleBarcodeReader::translateResultPoints(
		Ref<Result> result, int xOffset, int yOffset) {
	...
	for (int i = 0; i < oldResultPoints->size(); i++) {
		...
		newResultPoints->values().push_back(
				Ref<ResultPoint>(
						new ResultPoint(oldPoint->getX() + xOffset,
								oldPoint->getY() + yOffset)));
	}
	return Ref<Result>(
			new Result(result->getText(), result->getRawBytes(),
