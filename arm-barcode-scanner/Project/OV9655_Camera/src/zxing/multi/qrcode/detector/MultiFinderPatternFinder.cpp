...
vector<Ref<FinderPatternInfo> > MultiFinderPatternFinder::findMulti(
	...
	for (unsigned int i = 0; i < patternInfo.size(); i++) {
		...
		result.push_back(
				Ref<FinderPatternInfo>(new FinderPatternInfo(pattern)));
	}
	return result;
}
...
vector<vector<Ref<FinderPattern> > > MultiFinderPatternFinder::selectBestPatterns() {
	...
	for (int i1 = 0; i1 < (size - 2); i1++) {
		...
		for (int i2 = i1 + 1; i2 < (size - 1); i2++) {
			...
			float vModSize12 = (p1->getEstimatedModuleSize()
					- p2->getEstimatedModuleSize())
					/ min(p1->getEstimatedModuleSize(),
							p2->getEstimatedModuleSize());
			float vModSize12A = abs(
					p1->getEstimatedModuleSize()
							- p2->getEstimatedModuleSize());
			...
			for (int i3 = i2 + 1; i3 < size; i3++) {
				...
				// Compare the expected module sizes; if they are really off, skip
				float vModSize23 = (p2->getEstimatedModuleSize()
						- p3->getEstimatedModuleSize())
						/ min(p2->getEstimatedModuleSize(),
								p3->getEstimatedModuleSize());
				...
				Ref<FinderPatternInfo> info = Ref<FinderPatternInfo>(
						new FinderPatternInfo(test));
				float dA = FinderPatternFinder::distance(info->getTopLeft(),
						info->getBottomLeft());
				float dC = FinderPatternFinder::distance(info->getTopRight(),
						info->getBottomLeft());
				float dB = FinderPatternFinder::distance(info->getTopLeft(),
						info->getTopRight());
				// Check the sizes
