...
ByQuadrantReader::ByQuadrantReader(Reader& delegate) :
		delegate_(delegate) {
}
...
Ref<Result> ByQuadrantReader::decode(Ref<BinaryBitmap> image,
	...
	Ref<BinaryBitmap> topRight = image->crop(halfWidth, 0, halfWidth,
			halfHeight);
	try {
	...
	Ref<BinaryBitmap> bottomLeft = image->crop(0, halfHeight, halfWidth,
			halfHeight);
	try {
	...
	Ref<BinaryBitmap> bottomRight = image->crop(halfWidth, halfHeight,
	...
	Ref<BinaryBitmap> center = image->crop(quarterWidth, quarterHeight,
			halfWidth, halfHeight);
