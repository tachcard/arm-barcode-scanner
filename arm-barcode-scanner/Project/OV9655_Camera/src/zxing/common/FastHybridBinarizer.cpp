...
FastHybridBinarizer::FastHybridBinarizer(unsigned int width,
		unsigned int height) :
...
namespace {
inline int getBlackPointFromNeighbors(uint8_t *blackPoints, int subWidth, int x,
		int y) {
...
extern "C" void calculateBlockStatistics(uint32_t *results, uint8_t *block,
		uint32_t stride);
...
extern "C" void applyThreshold(uint8_t *out, uint8_t *in, uint32_t stride,
		uint32_t threshold);

void FastHybridBinarizer::thresholdBlock(uint8_t *out, uint8_t *block,
		int stride, int threshold) {
	/*int offset = 0;
	 int matrixOffset = 0;
...
void FastHybridBinarizer::calculateThresholdForBlockRow(uint8_t *blockRow,
		int y) {
	...
	for (int x = 0; x < subWidth; x++) {
		...
		thresholdBlock(&(matrix[location >> 3]), &(blockRow[xoffset]), width_,
				average);
	}
}
...
void FastHybridBinarizer::writeBlockRow(uint8_t *blockRow) {
	...
	for (unsigned int x = 0; x < width_; x += BLOCK_SIZE) {
		calculateBlackPoint(&(currentBlock[x]), x >> BLOCK_SIZE_POWER,
				blockRowsWritten);
	}
	blockRowsWritten++;
	...
	while ((blockRowsRead < numBlockRows) && (blockRowsWritten >= 5)
			&& ((blockRowsRead <= (blockRowsWritten - 3))
					|| (blockRowsRead >= (numBlockRows - 2)))) {
		calculateThresholdForBlockRow(&(workingRows[workingRowReadOffset]),
				blockRowsRead);
		blockRowsRead++;
...
// rio fix
int FastHybridBinarizer::getWidth() {
	return width_;
}

// rio fix
int FastHybridBinarizer::getHeight() {
	return height_;
}
