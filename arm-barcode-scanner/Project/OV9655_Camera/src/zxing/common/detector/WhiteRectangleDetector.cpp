...
WhiteRectangleDetector::WhiteRectangleDetector(Ref<BitMatrix> image) :
	...
	if (upInit_ < 0 || leftInit_ < 0 || downInit_ >= height_
			|| rightInit_ >= width_) {
...
Ref<ResultPoint> WhiteRectangleDetector::getBlackPointOnSegment(int aX_,
		int aY_, int bX_, int bY_) {
 ...
 *         leftmost and the third, the rightmost
 */
vector<Ref<ResultPoint> > WhiteRectangleDetector::centerEdges(
		Ref<ResultPoint> y, Ref<ResultPoint> z, Ref<ResultPoint> x,
		Ref<ResultPoint> t) {
 ...
 */
bool WhiteRectangleDetector::containsBlackPoint(int a, int b, int fixed,
		bool horizontal) {
	if (horizontal) {
		for (int x = a; x <= b; x++) {
