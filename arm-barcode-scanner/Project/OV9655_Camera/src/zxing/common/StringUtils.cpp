...
string StringUtils::guessEncoding(char* bytes, int length,
	...
	typedef char byte;
	boolean utf8bom = length > 3 && bytes[0] == (byte) 0xEF
			&& bytes[1] == (byte) 0xBB && bytes[2] == (byte) 0xBF;

	for (int i = 0; i < length && (canBeISO88591 || canBeShiftJIS || canBeUTF8);
			i++) {
		...
		if (canBeShiftJIS) {
			if (sjisBytesLeft > 0) {
				...
				if (sjisCurDoubleBytesWordLength
	...
	if (canBeShiftJIS
			&& (ASSUME_SHIFT_JIS || sjisMaxKatakanaWordLength >= 3
