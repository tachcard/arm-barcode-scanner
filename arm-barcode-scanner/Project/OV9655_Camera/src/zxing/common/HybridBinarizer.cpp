...
Ref<BitMatrix> HybridBinarizer::getBlackMatrix() {
	...
	if (width >= MINIMUM_DIMENSION && height >= MINIMUM_DIMENSION) {
		...
		ArrayRef<int> blackPoints = calculateBlackPoints(luminances, subWidth,
...
void HybridBinarizer::calculateThresholdForBlock(ArrayRef<char> luminances,
...
void HybridBinarizer::thresholdBlock(ArrayRef<char> luminances, int xoffset,
		int yoffset, int threshold, int stride, Ref<BitMatrix> const& matrix) {
	for (int y = 0, offset = yoffset * stride + xoffset; y < BLOCK_SIZE;
			y++, offset += stride) {
...
namespace {
inline int getBlackPointFromNeighbors(ArrayRef<int> blackPoints, int subWidth,
		int x, int y) {
		...
		for (int x = 0; x < subWidth; x++) {
			...
			for (int yy = 0, offset = yoffset * width + xoffset;
				...
				if (max - min > minDynamicRange) {
					...
					for (yy++, offset += width; yy < BLOCK_SIZE; yy++, offset +=
							width) {
			...
			if (max - min <= minDynamicRange) {
				average = min >> 1;
				if (y > 0 && x > 0) {
					int bp = getBlackPointFromNeighbors(blackPoints, subWidth,
							x, y);
					if (min < bp) {
						average = bp;
					}
