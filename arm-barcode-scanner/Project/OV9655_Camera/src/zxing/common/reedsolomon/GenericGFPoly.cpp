...
GenericGFPoly::GenericGFPoly(Ref<GenericGF> field, ArrayRef<int> coefficients) :
		field_(field) {
	...
	if (coefficientsLength > 1 && coefficients[0] == 0) {
		...
		if (firstNonZero == coefficientsLength) {
			...
			coefficients_ = ArrayRef<int>(
					new Array<int>(coefficientsLength - firstNonZero));
...
int GenericGFPoly::evaluateAt(int a) {
	...
	for (int i = 1; i < size; i++) {
		result = GenericGF::addOrSubtract(field_->multiply(a, result),
				coefficients_[i]);
	}
	return result;
}

Ref<GenericGFPoly> GenericGFPoly::addOrSubtract(
		Ref<zxing::GenericGFPoly> other) {
...
Ref<GenericGFPoly> GenericGFPoly::multiplyByMonomial(int degree,
		int coefficient) {
	if (degree < 0) {
...
std::vector<Ref<GenericGFPoly> > GenericGFPoly::divide(
		Ref<GenericGFPoly> other) {
	if (!(field_.object_ == other->field_.object_)) {
		throw IllegalArgumentException(
				"GenericGFPolys do not have same GenericGF field");
	}
	...
	while (remainder->getDegree() >= other->getDegree() && !remainder->isZero()) {
		...
		int scale = field_->multiply(
				remainder->getCoefficient(remainder->getDegree()),
				inverseDenominatorLeadingTerm);
		Ref<GenericGFPoly> term = other->multiplyByMonomial(degreeDifference,
				scale);
		Ref<GenericGFPoly> iterationQuotiont = field_->buildMonomial(
