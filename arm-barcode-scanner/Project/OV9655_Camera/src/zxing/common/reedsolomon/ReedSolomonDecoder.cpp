...
ReedSolomonDecoder::ReedSolomonDecoder(Ref<GenericGF> field_) :
		field(field_) {
}
...
vector<Ref<GenericGFPoly> > ReedSolomonDecoder::runEuclideanAlgorithm(
		Ref<GenericGFPoly> a, Ref<GenericGFPoly> b, int R) {
...
ArrayRef<int> ReedSolomonDecoder::findErrorLocations(
		Ref<GenericGFPoly> errorLocator) {
...
ArrayRef<int> ReedSolomonDecoder::findErrorMagnitudes(
		Ref<GenericGFPoly> errorEvaluator, ArrayRef<int> errorLocations) {
