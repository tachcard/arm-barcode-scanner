...
void GenericGF::initialize() {
	...
	zero = Ref<GenericGFPoly>(
			new GenericGFPoly(Ref<GenericGF>(this),
					ArrayRef<int>(new Array<int>(1))));
	zero->getCoefficients()[0] = 0;
	one = Ref<GenericGFPoly>(
			new GenericGFPoly(Ref<GenericGF>(this),
					ArrayRef<int>(new Array<int>(1))));
...
Ref<GenericGFPoly> GenericGF::buildMonomial(int degree, int coefficient) {
	...
	return Ref<GenericGFPoly>(
			new GenericGFPoly(Ref<GenericGF>(this), coefficients));
}
