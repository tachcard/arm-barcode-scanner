...
MultiFormatUPCEANReader::MultiFormatUPCEANReader(DecodeHints hints) :
		readers() {
...
Ref<Result> MultiFormatUPCEANReader::decodeRow(int rowNumber,
		Ref<BitArray> row) {
	// Compute this location once and reuse it on multiple implementations
	UPCEANReader::Range startGuardPattern = UPCEANReader::findStartGuardPattern(
			row);
	for (int i = 0, e = readers.size(); i < e; i++) {
		...
		// appropriate.
		bool ean13MayBeUPCA = result->getBarcodeFormat()
				== BarcodeFormat::EAN_13 && result->getText()->charAt(0) == '0';
		...
		if (ean13MayBeUPCA && canReturnUPCA) {
			...
			Ref<Result> resultUPCA(
					new Result(result->getText()->substring(1),
