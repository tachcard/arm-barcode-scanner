 ...
 */
const int L_PATTERNS_[][4] = { { 3, 2, 1, 1 }, // 0
		{ 2, 2, 2, 1 }, // 1
		{ 2, 1, 2, 2 }, // 2
		{ 1, 4, 1, 1 }, // 3
		{ 1, 1, 3, 2 }, // 4
 ...
 */
const int L_AND_G_PATTERNS_[][4] = { { 3, 2, 1, 1 }, // 0
		{ 2, 2, 2, 1 }, // 1
		{ 2, 1, 2, 2 }, // 2
		{ 1, 4, 1, 1 }, // 3
		{ 1, 1, 3, 2 }, // 4
...
const int UPCEANReader::MAX_AVG_VARIANCE =
		(int) (PATTERN_MATCH_RESULT_SCALE_FACTOR * 0.48f);
const int UPCEANReader::MAX_INDIVIDUAL_VARIANCE =
		(int) (PATTERN_MATCH_RESULT_SCALE_FACTOR * 0.7f);

#define VECTOR_INIT(v) v, v + sizeof(v)/sizeof(v[0])

const vector<int> UPCEANReader::START_END_PATTERN(
		VECTOR_INIT(START_END_PATTERN_));

const vector<int> UPCEANReader::MIDDLE_PATTERN(VECTOR_INIT(MIDDLE_PATTERN_));
const vector<int const*> UPCEANReader::L_PATTERNS(VECTOR_INIT(L_PATTERNS_));
const vector<int const*> UPCEANReader::L_AND_G_PATTERNS(
		VECTOR_INIT(L_AND_G_PATTERNS_));
...
Ref<Result> UPCEANReader::decodeRow(int rowNumber, Ref<BitArray> row,
	...
	resultPoints[0] = Ref<ResultPoint>(
			new OneDResultPoint(left, (float) rowNumber));
	resultPoints[1] = Ref<ResultPoint>(
			new OneDResultPoint(right, (float) rowNumber));
	Ref<Result> decodeResult(
			new Result(resultString, ArrayRef<char>(), resultPoints, format));
...
UPCEANReader::Range UPCEANReader::findGuardPattern(Ref<BitArray> row,
		int rowOffset, bool whiteFirst, vector<int> const& pattern) {
...
UPCEANReader::Range UPCEANReader::findGuardPattern(Ref<BitArray> row,
	...
	rowOffset =
			whiteFirst ?
					row->getNextUnset(rowOffset) : row->getNextSet(rowOffset);
...
int UPCEANReader::decodeDigit(Ref<BitArray> row, vector<int> & counters,
		int rowOffset, vector<int const*> const& patterns) {
