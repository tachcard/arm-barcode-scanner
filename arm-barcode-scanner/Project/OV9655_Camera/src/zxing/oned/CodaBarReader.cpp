 ...
 */
const int CHARACTER_ENCODINGS[] = { 0x003, 0x006, 0x009, 0x060, 0x012, 0x042,
		0x021, 0x024, 0x030, 0x048, // 0-9
...
const int CodaBarReader::PADDING = (int) (PATTERN_MATCH_RESULT_SCALE_FACTOR
		* 1.5f);
...
Ref<Result> CodaBarReader::decodeRow(int rowNumber, Ref<BitArray> row) {
	...
	return Ref<Result>(
			new Result(Ref<String>(new String(decodeRowResult)),
					ArrayRef<char>(), resultPoints, BarcodeFormat::CODABAR));
}
