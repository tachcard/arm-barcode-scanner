...
Ref<Result> UPCAReader::decodeRow(int rowNumber, Ref<BitArray> row,
	...
	return maybeReturnResult(
			ean13Reader.decodeRow(rowNumber, row, startGuardRange));
}
...
int UPCAReader::decodeMiddle(Ref<BitArray> row, Range const& startRange,
...
Ref<Result> UPCAReader::maybeReturnResult(Ref<Result> result) {
	...
	if (text[0] == '0') {
		...
		Ref<Result> res(
				new Result(resultString, result->getRawBytes(),
