...
#include <zxing/datamatrix/detector/DatamatrixDetector.h>
...
Ref<DetectorResult> Detector::detect() {
	Ref<WhiteRectangleDetector> rectangleDetector_(
			new WhiteRectangleDetector(image_));
	...
	for (PointMap::const_iterator entry = pointCount.begin(), end =
			pointCount.end(); entry != end; ++entry) {
	...
	if (4 * dimensionTop >= 7 * dimensionRight
		...
		transform = createTransform(topLeft, correctedTopRight, bottomLeft,
				bottomRight, dimensionTop, dimensionRight);
		...
		bits = sampleGrid(image_, dimensionCorrected, dimensionCorrected,
				transform);
	}
 ...
 */
Ref<ResultPoint> Detector::correctTopRightRectangular(
		Ref<ResultPoint> bottomLeft, Ref<ResultPoint> bottomRight,
		Ref<ResultPoint> topLeft, Ref<ResultPoint> topRight, int dimensionTop,
		int dimensionRight) {
	...
	Ref<ResultPoint> c1(
			new ResultPoint(topRight->getX() + corr * cos,
...
Ref<ResultPoint> Detector::correctTopRight(Ref<ResultPoint> bottomLeft,
		...
		Ref<ResultPoint> topRight, int dimension) {
...
Ref<ResultPointsAndTransitions> Detector::transitionsBetween(
		Ref<ResultPoint> from, Ref<ResultPoint> to) {
	...
	Ref<ResultPointsAndTransitions> result(
			new ResultPointsAndTransitions(from, to, transitions));
	return result;
}
...
Ref<PerspectiveTransform> Detector::createTransform(Ref<ResultPoint> topLeft,
		...
		Ref<ResultPoint> bottomRight, int dimensionX, int dimensionY) {

	Ref<PerspectiveTransform> transform(
			PerspectiveTransform::quadrilateralToQuadrilateral(0.5f, 0.5f,
					dimensionX - 0.5f, 0.5f, dimensionX - 0.5f,
					dimensionY - 0.5f, 0.5f, dimensionY - 0.5f, topLeft->getX(),
...
Ref<BitMatrix> Detector::sampleGrid(Ref<BitMatrix> image, int dimensionX,
		int dimensionY, Ref<PerspectiveTransform> transform) {
	GridSampler &sampler = GridSampler::getInstance();
	return sampler.sampleGrid(image, dimensionX, dimensionY, transform);
}

void Detector::insertionSort(
		std::vector<Ref<ResultPointsAndTransitions> > &vector) {
	int max = vector.size();
	bool swapped = true;
...
int Detector::compare(Ref<ResultPointsAndTransitions> a,
		Ref<ResultPointsAndTransitions> b) {
