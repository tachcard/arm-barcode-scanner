...
#include <zxing/datamatrix/decoder/DatamatrixBitMatrixParser.h>
...
BitMatrixParser::BitMatrixParser(Ref<BitMatrix> bitMatrix) :
		bitMatrix_(NULL), parsedVersion_(NULL), readBitMatrix_(NULL) {
	...
	readBitMatrix_ = new BitMatrix(bitMatrix_->getWidth(),
			bitMatrix_->getHeight());
}
...
Ref<Version> BitMatrixParser::readVersion(Ref<BitMatrix> bitMatrix) {
	...
	Ref<Version> version = parsedVersion_->getVersionForDimensions(numRows,
			numColumns);
	if (version != 0) {
		return version;
	}
		...
		if ((row == numRows) && (column == 0) && !corner1Read) {
			...
			do {
				if ((row < numRows) && (column >= 0)
					...
					result[resultOffset++] = (char) readUtah(row, column,
							numRows, numColumns);
				}
				row -= 2;
				column += 2;
...
bool BitMatrixParser::readModule(int row, int column, int numRows,
		int numColumns) {
...
int BitMatrixParser::readUtah(int row, int column, int numRows,
		int numColumns) {
	int currentByte = 0;
...
Ref<BitMatrix> BitMatrixParser::extractDataRegion(Ref<BitMatrix> bitMatrix) {
	...
	for (int dataRegionRow = 0; dataRegionRow < numDataRegionsRow;
			++dataRegionRow) {
		...
		for (int dataRegionColumn = 0; dataRegionColumn < numDataRegionsColumn;
			...
			for (int i = 0; i < dataRegionSizeRows; ++i) {
				int readRowOffset = dataRegionRow * (dataRegionSizeRows + 2) + 1
						+ i;
