...
const char DecodedBitStreamParser::ALPHANUMERIC_CHARS[] = { '0', '1', '2', '3',
...
void DecodedBitStreamParser::decodeByteSegment(Ref<BitSource> bits_,
		string& result, int count, CharacterSetECI* currentCharacterSetECI,
...
void DecodedBitStreamParser::decodeAlphanumericSegment(Ref<BitSource> bits_,
		string& result, int count, bool fc1InEffect) {
	BitSource& bits(*bits_);
	ostringstream bytes;
...
Ref<DecoderResult> DecodedBitStreamParser::decode(ArrayRef<char> bytes,
		Version* version, ErrorCorrectionLevel const& ecLevel,
			...
			if (mode != &Mode::TERMINATOR) {
				if ((mode == &Mode::FNC1_FIRST_POSITION)
					...
					if (mode == &Mode::HANZI) {
						...
						int count = bits.readBits(
								mode->getCharacterCountBits(version));
						if (mode == &Mode::NUMERIC) {
							decodeNumericSegment(bits_, result, count);
						} else if (mode == &Mode::ALPHANUMERIC) {
							decodeAlphanumericSegment(bits_, result, count,
									fc1InEffect);
						} else if (mode == &Mode::BYTE) {
							decodeByteSegment(bits_, result, count,
	...
	return Ref<DecoderResult>(
			new DecoderResult(bytes, Ref<String>(new String(result)),
