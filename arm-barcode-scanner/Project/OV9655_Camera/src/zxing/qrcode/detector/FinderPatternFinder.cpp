...
float FinderPatternFinder::crossCheckVertical(std::size_t startI,
		std::size_t centerJ, int maxCount, int originalStateCountTotal) {
	...
	return foundPatternCross(stateCount) ?
			centerFromEnd(stateCount, i) : std::nan("");
}

float FinderPatternFinder::crossCheckHorizontal(std::size_t startJ,
		std::size_t centerI, int maxCount, int originalStateCountTotal) {
	...
	return foundPatternCross(stateCount) ?
			centerFromEnd(stateCount, j) : std::nan("");
}

bool FinderPatternFinder::handlePossibleCenter(int* stateCount, std::size_t i,
		std::size_t j) {
	int stateCountTotal = stateCount[0] + stateCount[1] + stateCount[2]
			+ stateCount[3] + stateCount[4];
	float centerJ = centerFromEnd(stateCount, j);
	float centerI = crossCheckVertical(i, (std::size_t) centerJ, stateCount[2],
			stateCountTotal);
	if (!isnan(centerI)) {
		// Re-cross check
		centerJ = crossCheckHorizontal((std::size_t) centerJ,
				(std::size_t) centerI, stateCount[2], stateCountTotal);
		if (!isnan(centerJ)) {
			...
			for (std::size_t index = 0; index < max; index++) {
				...
				if (center->aboutEquals(estimatedModuleSize, centerI,
						centerJ)) {
					possibleCenters_[index] = center->combineEstimate(centerI,
...
vector<Ref<FinderPattern> > FinderPatternFinder::selectBestPatterns() {
	...
	if (startSize > 3) {
		...
		float stdDev = (float) std::sqrt(
				square / startSize - average * average);
		...
		for (size_t i = 0;
				i < possibleCenters_.size() && possibleCenters_.size() > 3;
				i++) {
			if (abs(possibleCenters_[i]->getEstimatedModuleSize() - average)
					> limit) {
	...
	if (possibleCenters_.size() > 3) {
		...
		sort(possibleCenters_.begin(), possibleCenters_.end(),
				CenterComparator(average));
	}

	if (possibleCenters_.size() > 3) {
		possibleCenters_.erase(possibleCenters_.begin() + 3,
...
FinderPatternFinder::FinderPatternFinder(Ref<BitMatrix> image,
		...
		image_(image), possibleCenters_(), hasSkipped_(false), callback_(
				callback) {
}
				...
				if ((currentState & 1) == 0) { // Counting black pixels
					if (currentState == 4) { // A winner?
						if (foundPatternCross(stateCount)) { // Yes
							bool confirmed = handlePossibleCenter(stateCount, i,
									j);
							if (confirmed) {
