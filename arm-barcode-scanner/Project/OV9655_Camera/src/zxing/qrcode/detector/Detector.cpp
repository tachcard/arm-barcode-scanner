...
Ref<DetectorResult> Detector::processFinderPatternInfo(
	...
	Version *provisionalVersion = Version::getProvisionalVersionForDimension(
			dimension);
	int modulesBetweenFPCenters = provisionalVersion->getDimensionForVersion()
			- 7;
	...
	if (provisionalVersion->getAlignmentPatternCenters().size() > 0) {
		...
		float correctionToTopLeft = 1.0f
				- 3.0f / (float) modulesBetweenFPCenters;
	...
	Ref<PerspectiveTransform> transform = createTransform(topLeft, topRight,
			bottomLeft, alignmentPattern, dimension);
	Ref<BitMatrix> bits(sampleGrid(image_, dimension, transform));
	ArrayRef<Ref<ResultPoint> > points(
			new Array<Ref<ResultPoint> >(alignmentPattern == 0 ? 3 : 4));
...
Ref<PerspectiveTransform> Detector::createTransform(Ref<ResultPoint> topLeft,
	...
	Ref<PerspectiveTransform> transform(
			PerspectiveTransform::quadrilateralToQuadrilateral(3.5f, 3.5f,
					dimMinusThree, 3.5f, sourceBottomRightX, sourceBottomRightY,
					3.5f, dimMinusThree, topLeft->getX(), topLeft->getY(),
					topRight->getX(), topRight->getY(), bottomRightX,
					bottomRightY, bottomLeft->getX(), bottomLeft->getY()));
...
int Detector::computeDimension(Ref<ResultPoint> topLeft,
		Ref<ResultPoint> topRight, Ref<ResultPoint> bottomLeft,
		float moduleSize) {
	int tltrCentersDimension = MathUtils::round(
			ResultPoint::distance(topLeft, topRight) / moduleSize);
	int tlblCentersDimension = MathUtils::round(
...
float Detector::sizeOfBlackWhiteBlackRunBothWays(int fromX, int fromY, int toX,
		int toY) {
	...
	if (otherToX < 0) {
		...
		scale = (float) (image_->getWidth() - 1 - fromX)
	...
	if (otherToY < 0) {
		...
		scale = (float) (image_->getHeight() - 1 - fromY)
...
float Detector::sizeOfBlackWhiteBlackRun(int fromX, int fromY, int toX,
		int toY) {
...
Ref<AlignmentPattern> Detector::findAlignmentInRegion(
	...
	AlignmentPatternFinder alignmentFinder(image_, alignmentAreaLeftX,
