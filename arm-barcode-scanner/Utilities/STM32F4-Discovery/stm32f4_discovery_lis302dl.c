...
void LIS302DL_Init(LIS302DL_InitTypeDef *LIS302DL_InitStruct) {
    ...
    ctrl = (uint8_t)(
            LIS302DL_InitStruct->Output_DataRate
...
void LIS302DL_FilterConfig(
    ...
    ctrl |=
            (uint8_t)(
                    LIS302DL_FilterConfigStruct->HighPassFilter_Data_Selection
...
void LIS302DL_InterruptConfig(
    ...
    ctrl = (uint8_t)(
            LIS302DL_IntConfigStruct->Latch_Request
 ...
 * @retval None
 */
void LIS302DL_Write(uint8_t* pBuffer, uint8_t WriteAddr,
...
void LIS302DL_ReadACC(int32_t* out) {
    ...
    case 0x00:
        for (i = 0; i < 0x03; i++) {
            *out = (int32_t)(
                    LIS302DL_SENSITIVITY_2_3G * (int8_t) buffer[2 * i]);
            out++;
        }
        break;
    ...
    case 0x20:
        for (i = 0; i < 0x03; i++) {
            *out = (int32_t)(
                    LIS302DL_SENSITIVITY_9_2G * (int8_t) buffer[2 * i]);
            out++;
        }
        break;
    default:
