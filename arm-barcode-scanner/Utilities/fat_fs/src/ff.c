...
const XCHAR *path /* Full-path string to find a file or directory */
    ...
    if ((UINT) *path < ' ') { /* Null path means the start directory itself */
        ...
        for (;;) {
            ...
            dj->sclust = ((DWORD) LD_WORD(dir+DIR_FstClusHI) << 16)
...
FRESULT f_unlink(const XCHAR *path /* Pointer to the file or directory path */
    ...
    if (dir[DIR_Attr] & AM_RDO) /* Is it a R/O object? */
        LEAVE_FF(dj.fs, FR_DENIED);
    dclst = ((DWORD) LD_WORD(dir+DIR_FstClusHI) << 16)
            | LD_WORD(dir+DIR_FstClusLO);
    ...
    if (res == FR_NO_FILE) { /* Is it a valid path and no name collision? */
        ...
        if (res == FR_OK) {
            ...
            if (dir[DIR_Attr] & AM_DIR) { /* Update .. entry in the directory if needed */
                dw =
                        clust2sect(dj_new.fs,
                                (DWORD) LD_WORD(
                                        dir+DIR_FstClusHI) | LD_WORD(dir+DIR_FstClusLO));
                if (!dw) {
                    res = FR_INT_ERR;
                } else {
                    ...
                    if (res == FR_OK && dir[1] == '.') {
                        dw = (dj_new.fs->fs_type == FS_FAT32
                                && dj_new.sclust == dj_new.fs->dirbase) ?
                                0 : dj_new.sclust;
...
WORD allocsize /* Allocation unit size [bytes] */
    ...
    static const DWORD sstbl[] = { 2048000, 1024000, 512000, 256000, 128000,
            64000, 32000, 16000, 8000, 4000, 0 };
    static const WORD cstbl[] = { 32768, 16384, 8192, 4096, 2048, 16384, 8192,
            4096, 2048, 1024, 512 };
    BYTE fmt, m, *tbl;
    ...
    return (disk_ioctl(drv, CTRL_SYNC, (void*) NULL) == RES_OK) ?
            FR_OK : FR_DISK_ERR;
}
