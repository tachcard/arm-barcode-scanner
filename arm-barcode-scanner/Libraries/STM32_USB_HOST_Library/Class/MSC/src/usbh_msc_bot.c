...
uint8_t USBH_MSC_DecodeCSW(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost) {
    ...
    if (HCD_IsDeviceConnected(pdev)) {
        ...
        if (dataXferCount != USBH_MSC_CSW_LENGTH) {
            ...
            if (USBH_MSC_CSWData.field.CSWSignature
                    == USBH_MSC_BOT_CSW_SIGNATURE) {/* Check Condition 1. dCSWSignature is equal to 53425355h */

                if (USBH_MSC_CSWData.field.CSWTag
                        == USBH_MSC_CBWData.field.CBWTag) {
                    ...
                    if (USBH_MSC_CSWData.field.CSWStatus == USBH_MSC_OK) {
                        ...
                        status = USBH_MSC_OK;
                    } else if (USBH_MSC_CSWData.field.CSWStatus
                            == USBH_MSC_FAIL) {
                        status = USBH_MSC_FAIL;
                    }
