...
uint8_t USBH_MSC_TestUnitReady(USB_OTG_CORE_HANDLE *pdev) {
    ...
    if (HCD_IsDeviceConnected(pdev)) {
        ...
        case CMD_WAIT_STATUS:
            ...
            else if (USBH_MSC_BOTXferParam.BOTXferStatus
...
uint8_t USBH_MSC_ReadCapacity10(USB_OTG_CORE_HANDLE *pdev) {
    ...
    if (HCD_IsDeviceConnected(pdev)) {
        ...
        case CMD_WAIT_STATUS:
            if (USBH_MSC_BOTXferParam.BOTXferStatus == USBH_MSC_OK) {
                /*assign the capacity*/
                (((uint8_t*) &USBH_MSC_Param.MSCapacity)[3]) =
                        USBH_DataInBuffer[0];
                (((uint8_t*) &USBH_MSC_Param.MSCapacity)[2]) =
                        USBH_DataInBuffer[1];
                (((uint8_t*) &USBH_MSC_Param.MSCapacity)[1]) =
                        USBH_DataInBuffer[2];
                (((uint8_t*) &USBH_MSC_Param.MSCapacity)[0]) =
                        USBH_DataInBuffer[3];

                /*assign the page length*/
                (((uint8_t*) &USBH_MSC_Param.MSPageLength)[1]) =
                        USBH_DataInBuffer[6];
                (((uint8_t*) &USBH_MSC_Param.MSPageLength)[0]) =
                        USBH_DataInBuffer[7];
...
uint8_t USBH_MSC_RequestSense(USB_OTG_CORE_HANDLE *pdev) {
    ...
    if (HCD_IsDeviceConnected(pdev)) {
        ...
        case CMD_SEND_STATE:
            ...
            USBH_MSC_BOTXferParam.pRxTxBuff = USBH_DataInBuffer;
            USBH_MSC_BOTXferParam.MSCStateBkp =
        ...
        case CMD_WAIT_STATUS:
            ...
            if (USBH_MSC_BOTXferParam.BOTXferStatus == USBH_MSC_OK) {
                /* Get Sense data*/
                (((uint8_t*) &USBH_MSC_Param.MSSenseKey)[3]) =
                        USBH_DataInBuffer[0];
                (((uint8_t*) &USBH_MSC_Param.MSSenseKey)[2]) =
                        USBH_DataInBuffer[1];
                (((uint8_t*) &USBH_MSC_Param.MSSenseKey)[1]) =
                        USBH_DataInBuffer[2];
                (((uint8_t*) &USBH_MSC_Param.MSSenseKey)[0]) =
                        USBH_DataInBuffer[3];
 ...
 * @retval Status
 */
uint8_t USBH_MSC_Write10(USB_OTG_CORE_HANDLE *pdev, uint8_t *dataBuffer,
        uint32_t address, uint32_t nbOfbytes) {
    uint8_t index;
    USBH_MSC_Status_TypeDef status = USBH_MSC_BUSY;
    uint16_t nbOfPages;
 ...
 * @retval Status
 */
uint8_t USBH_MSC_Read10(USB_OTG_CORE_HANDLE *pdev, uint8_t *dataBuffer,
        uint32_t address, uint32_t nbOfbytes) {
    uint8_t index;
