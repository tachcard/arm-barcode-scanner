...
static int8_t SCSI_CheckAddressRange(uint8_t lun, uint32_t blk_offset,
        uint16_t blk_nbr);
...
static int8_t SCSI_Inquiry(uint8_t lun, uint8_t *params) {
    ...
    if (params[1] & 0x01)/*Evpd is set*/
    {
        ...
        pPage = (uint8_t *) &USBD_STORAGE_fops->pInquiry[lun
...
static int8_t SCSI_Read10(uint8_t lun, uint8_t *params) {
    if (MSC_BOT_State == BOT_IDLE) /* Idle */
    {
        ...
        SCSI_blk_addr = (params[2] << 24) | (params[3] << 16) | (params[4] << 8)
                | params[5];
...
static int8_t SCSI_Write10(uint8_t lun, uint8_t *params) {
    if (MSC_BOT_State == BOT_IDLE) /* Idle */
    {
        ...
        MSC_OUT_EP, MSC_BOT_Data, MIN(SCSI_blk_len, MSC_MEDIA_PACKET));
