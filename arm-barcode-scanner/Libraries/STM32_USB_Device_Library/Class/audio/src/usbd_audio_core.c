...
USBD_Class_cb_TypeDef AUDIO_cb = { usbd_audio_Init, usbd_audio_DeInit,
        usbd_audio_Setup,
        NULL, /* EP0_TxSent */
...
static uint8_t usbd_audio_Init(void *pdev, uint8_t cfgidx) {
    ...
    DCD_EP_PrepareRx(pdev, AUDIO_OUT_EP, (uint8_t*) IsocOutBuff,
    AUDIO_OUT_PACKET);
...
static uint8_t usbd_audio_DataOut(void *pdev, uint8_t epnum) {
    if (epnum == AUDIO_OUT_EP) {
        ...
        if (IsocOutWrPtr
                >= (IsocOutBuff + (AUDIO_OUT_PACKET * OUT_PACKET_NUM))) {/* All buffers are full: roll back */
        ...
        DCD_EP_PrepareRx(pdev, AUDIO_OUT_EP, (uint8_t*) (IsocOutWrPtr),
        AUDIO_OUT_PACKET);
        ...
        if ((PlayFlag == 0)
                && (IsocOutWrPtr
                        >= (IsocOutBuff
                                + ((AUDIO_OUT_PACKET * OUT_PACKET_NUM) / 2)))) {
...
static uint8_t usbd_audio_SOF(void *pdev) {
    ...
    if (PlayFlag) {
        ...
        if (IsocOutRdPtr
                >= (IsocOutBuff + (AUDIO_OUT_PACKET * OUT_PACKET_NUM))) {/* Roll back to the start of buffer */
